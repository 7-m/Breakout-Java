...
public class Main2 {
	...
	private void init() {
		...
		final Paddle paddle = new Paddle(0.2, 0.02);
		final Ball ball = new Ball(0.03, 0.03, 0.5, 0.1);
		int ncols = 20;
		int nrow = 10;
		...
		for (int i = 0; i < nrow; i++) {
			for (int j = 0; j < ncols; j++) {
				if (Math.random() > 0.5)
					bricks[i][j] = new Brick(width, height, j * width, (0.8 - height * i));
			}
		}
		GameContext gtx = new GameContext(bricks, paddle, ball, ncols * nrow);
